...
output$mapFilterPlot <- renderDataLite({
   selectVals <- input$mapFilterSelect
   cdo <- exposedCogDF()
   if(!is.null(selectVals) && !is.null(cdo)) {
      getMapPlotDat(cdo, selectVals$varName, distType = selectVals$distType, plotType = selectVals$plotType, maxLevels = 10000)
   }
})
